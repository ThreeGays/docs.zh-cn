...
workingDir = "/home/disk1/hf/website-docs/docs/en-us/master/"
entry_file = workingDir + "TOC.md"
...
for tp, lv, f in followups:
    if tp != 'FILE':
        continue
    try:
        for line in open(workingDir + f).readlines():
            if line.startswith("#"):
                tag = line.strip()
                break
...
def replace_link_wrap(chapter, name):
    ...
    def replace_link(match):
        ...
        if link.endswith('.md') or '.md#' in link:
            ...
            img_link = re.sub(r'[\.\/]*assets\/', './assets/', link, count=0, flags=0)
...
for type_, level, name in followups:
    if type_ == 'TOC':
        ...
        try:
            with open(workingDir + name) as fp:
                chapter = fp.read()
